(function(){
  ...
  var add_round_key = function(block, offset, key){
    var block_view = new Uint32Array(block);
    var key_view = new Uint32Array(key, offset*16);
    var i, n;
    n = block_view.length;
    for(i=0; i<n; ++i)
      block_view[i] ^= key_view[i];
  };
  ...
  var encrypt_block = function(plain, scheduled_key){
    var plain_view8 = new Uint8Array(plain);
    var cipher = new ArrayBuffer(16);
    ...
    add_round_key(cipher, 0, scheduled_key);

    for(i=1; i<=round_end; ++i){
      sub_bytes(cipher);
      shift_row(cipher);
      mix_column(cipher);
      add_round_key(cipher, i, scheduled_key);
    }

    sub_bytes(cipher);
    shift_row(cipher);
    add_round_key(cipher, round_end+1, scheduled_key);

    return cipher;
  };
  var decrypt_block = function(cipher, scheduled_key){
    var cipher_view8 = new Uint8Array(cipher);
    var plain = new ArrayBuffer(16);
    var plain_view8 = new Uint8Array(plain);
    var round_end;
    if( scheduled_key.byteLength >= 240 )
      round_end = 13;
    ...
    add_round_key_inv(plain, round_end+1, scheduled_key);
    shift_row_inv(plain);
    sub_bytes_inv(plain);

    for(i=round_end; i>=1; --i){
      add_round_key_inv(plain, i, scheduled_key);
      mix_column_inv(plain);
      shift_row_inv(plain);
      sub_bytes_inv(plain);
    }

    add_round_key_inv(plain, 0, scheduled_key);

    return plain;
  };

  window.key_schedule = key_schedule;
  window.encrypt_block = encrypt_block;
  window.decrypt_block = decrypt_block;
})();
